...
// One Page (for now?)
// - Ticket Login (Active Event Only)

// Sidebar Information:
// - avatar_url
// - name (first_name last_name)
// - email
// - ticket type (Speaker, Staff, Attendee, etc)
//      Use a Badge to style this
//      Maybe something which detects Talkshow Ticket with same email?
//      Upsell if missing? Pre-fill form with email from current ticket
//      We'll need to support custom metadata to support mismatched emails
//      Add a '?' button which explains in a modal if their talkshow pass is missing then to use same email

// Sidebar Navigation:
// Many of these pages are likely to be hidden if the user is a live stream pass holder
// Can we use ScrollSpy to highlight the current section?
// Tapping on links should scroll to that section
//
// - Personal Information (Pronouns, Job Title, Social URLs)
//      https://silicon.createx.studio/components/forms.html
//      Save button (toast)
// - Merch Preferences (Toggle On/Off, Shirt Size)
//      noUISlider component (https://refreshless.com/nouislider/examples/#section-pips) with pips for different shirt sizes
//      Save button (toast)
// - Drop-in Sessions
// - Focus Groups
//      Drop-in sessions and focus groups will be styled carousels, copy will be different
// - VISA Invitation
//      Do we automate this? (Talk to Adam) or just explain the process, and another pre-filled email button
// - Refund Ticket
//      "We will refund 100% of your ticket cost, minus card processing fees, if requested before the 14th of August 2024."
//      Date will be calculated automatically based on date of conference minus X days (determined by Adam).
//      Button will initiate email to info@swiftleeds.co.uk with subject "Refund Request" and ticket information in body
// - Logout (External Link)

// Need to fix the drop-in session print out too
