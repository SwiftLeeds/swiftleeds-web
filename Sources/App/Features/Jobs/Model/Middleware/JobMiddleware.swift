...
struct JobMiddleware: AsyncModelMiddleware {
    func create(model: Job, on db: Database, next: AnyAsyncModelResponder) async throws {
        try await next.create(model, on: db)
        try await updateTimestamp(on: db, title: model.title)
    }

    func update(model: Job, on db: Database, next: AnyAsyncModelResponder) async throws {
        try await next.update(model, on: db)
        try await updateTimestamp(on: db, title: model.title)
    }

    func delete(model: Job, force: Bool, on db: Database, next: AnyAsyncModelResponder) async throws {
        try await next.delete(model, force: force, on: db)
        try await updateTimestamp(on: db, title: model.title)
    }

    private func updateTimestamp(on db: Database, title: String) async throws {
